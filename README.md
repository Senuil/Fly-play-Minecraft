...
新增一格台階跳躍、前方坑洞／危險方塊停止、卡住時原地轉向，以及跟隨指定玩家。
這些是包在神經模型外的工程輔助；**神經子圖仍為 6 個神經元，沒有新增學習能力**。
操作和限制見 [玩家模式教學](docs/player-guide.md)。啟動方式與 v0.1 相同。
...
v0.2 增加有限的前方落差／危險方塊檢查，但不是完整避險保證；尚無採礦、戰鬥或全域尋路。
...
| `bridge/player.mjs` | 工程輔助：地形檢查、跳躍、脫困、跟隨及終端指令 |
...
神經讀出仍為前進與 yaw；v0.2 的跳躍與恢復轉向由玩家輔助層處理。不衝刺、不操作物品。
